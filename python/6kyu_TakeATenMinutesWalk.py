...
# laid out in a perfect grid. You arrived ten minutes too early to 
# an appointment, so you decided to take the opportunity to 
# go for a short walk. The city provides its citizens with 
# a Walk Generating App on their phones -- everytime you 
# press the button it sends you an array of one-letter strings 
...
def is_valid_walk(walk):
    ...
    east_west = walk.count('w') == walk.count('e')
    if len(walk) == 10 and north_south and east_west:
        return True
    else:
        return False